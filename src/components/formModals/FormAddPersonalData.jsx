...
export default function AddAccount() {
    ...
    useEffect(() => {
        // Verificar si los datos del usuario ya están completos
    ...
    const saveAccount = async (e) => {
        ...
        try {
            ...
            setIsUserDataComplete(true); // Actualizar el estado para no mostrar el formulario nuevamente
        } catch (error) {
            setLoader('');
            setAlerta('Error de datos!');
        }
    ...
    if (isUserDataComplete) {
        return null; // No mostrar el formulario si los datos ya están completos
    }
    ...
    return (
        <div className='fixed flex justify-center items-center top-0 left-0 bg-[#0000007c] h-screen w-screen z-40' onClick={() => setModal('')}>
            <div className='relative flex flex-col items-start justify-center bg-gray-200 w-[450px] h-[450px] p-5 px-12 space-y-3 rounded-[5px]' onClick={(e) => e.stopPropagation()}>
                ...
                <button type="button"
                    className="w-[300px] relative left-0 right-0 mx-auto text-white bg-gradient-to-br from-blue-600 to-blue-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-[10px] px-5 py-1.5 text-center mb-2"
