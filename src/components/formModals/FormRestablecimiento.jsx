...
export default function Modal({ children, funcion, alert, cancelText, successText, seccion }) {
            ...
            try {
                ...
                const response = await fetch(
                    window?.location?.href?.includes('localhost')
                        ? `http://localhost:3000/api/authSystem/auth/register/${i._id}`
                        : `https://api.fastcash-mx.com/api/authSystem/auth/register/${i._id}`, {
    ...
    const restablecimientoIndividual = async () => {
        try {
            ...
            const response = await fetch(
                window?.location?.href?.includes('localhost')
                    ? `http://localhost:3000/api/authSystem/auth/register/${checkedArr[0]._id}`
                    : `https://api.fastcash-mx.com/api/authSystem/auth/register/${checkedArr[0]._id}`, {
                method: 'PUT', // El método es PUT para actualizar
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`, // Si estás usando JWT
