...
export default function TableComisionVerification () {
    ...
    async function handlerFetch(limit, page) {
      ...
      const baseUrl = window?.location?.href?.includes("localhost")
        ? `http://localhost:3007/api/loansBuckup/getCasosVerificador?email=${user.email}&limit=${limit}&page=${page}`
        : `https://api.fastcash-mx.com/api/loansBuckup/getCasosVerificador?email=${user.email}limit=${limit}&page=${page}`;
    ...
    async function handlerFetchComision() {
      ...
      const result = await res.json();
      console.log("url comision: ", result);
      setFilter_1(result[0]);
    }
          ...
          <table className="w-full min-w-[1000px] border-[1px] bg-white text-[14px] text-left text-gray-500 border-t-4 border-t-gray-400">
            <thead className="text-[10px] text-white uppercase bg-slate-200 sticky top-[0px] z-20">
              <tr className=" bg-slate-200">
                ...
                <th className="px-4 py-2 text-gray-700">CASOS APROBADOS</th>
              ...
              {data.data?.map((i, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-2">
                    {i.cantidadCasosDispersados}</td>
                  <td className="px-4 py-2">
                    {i.cuenta.length > 0
                      ? ((i.cantidadCasosDispersados / i.cantidadDeCasosAsignados) * 100).toFixed(2) + "%"
                      : "0%"}
  
                  </td>
                  <td className="px-4 py-2">
                    {filter_1?.comisionPorAprobacion}
                  </td>
                  <td className="px-4 py-2">
                    {(filter_1?.comisionPorAprobacion * (i.cantidadCasosDispersados))}
                  </td>
                </tr>
              ))}
            </tbody>
