    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch(window?.location?.href.includes('localhost')
                ...
                : `https://api.fastcash-mx.com/api/verification/${id}`)
    ...
    return (
        <div className="bg-gray-300 min-h-screen w-full flex justify-center items-center">
            {(seccion?.toLowerCase() === "payment" || seccion?.toLowerCase() === "extension") && item === "data" ? (
                <div className="w-[24%] mx-auto p-4 m-4 bg-white bg-gradient-to-b from-blue-500 to-blue-600 rounded">
                    <main className="flex-grow">
                        ...
                        {/* Payment Amount Card */}
                        <div className="bg-blue-600 text-white p-3 mb-3 rounded-lg">
                            <div className="flex items-center gap-3">
                                <div>
                                    <div className="text-sm opacity-90 capitalize">{seccion === 'payment' ? 'Monto de prestamo' : 'Pago para extension'}</div>
                                    <div className="text-lg font-semibold">$ {seccion === 'payment' ? itemData?.cantidadDispersada : itemData?.valorExtencion}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-3 mb-4 rounded-lg">
                            ...
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <div className="text-gray-500 ">{itemData?.idDeSubFactura}</div>
                            </div>
                        </div>
                    </main>
                    ...
                    <Link
                        ...
                        <button className="w-full bg-orange-400 hover:bg-orange-500 text-white py-4 text-base rounded-full mb-4">
                            Confirmar
                        </button>
                    </Link>
                </div>
                ...
                <div className="max-w-md mx-auto p-4 m-4 bg-white bg-gradient-to-b from-blue-500 to-blue-600 rounded">
                    ...
                    <div className="mb-8">
                        ...
                        <div className="flex items-baseline gap-1 mt-1">
                            <span className="text-3xl font-bold text-gray-950">$ { seccion === "payment" ? itemData?.cantidadDispersada : itemData?.valorExtencion}</span>
                        </div>
                        <div className="mt-2">
                            <p className="text-sm text-gray-100">Código de pago</p>
                            <div className="flex items-center justify-between gap-2">
                                <code className="text-sm bg-gray-50 px-2 py-1 rounded text-gray-950">{itemData?.idDeSubFactura}</code>
                                <button
                                    onClick={handleCopy}
                    ...
                    <div className="relative pl-10 space-y-8">
                        {["Abre tu banca móvil", "Pegar el código de pago", "Ingresar el monto"].map((step, index) => (
